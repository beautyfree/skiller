# Skiller — design system (Linear-inspired)
...
## 10. Skiller implementation map


| Token / rule                    | CSS variable / location                                                                                   |
| ------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Canvas (dark)                   | `--background` → `#08090a`                                                                                |
| Sidebar / panel rail            | `--sidebar` → `#0f1011`                                                                                   |
| Main shell & cards              | `--card` → `#191a1b`                                                                                      |
| Hover surface                   | `--ds-surface-hover` → `rgba(255,255,255,0.06)` (solid steps where needed: `--ds-bg-secondary` `#28282c`) |
| Primary text                    | `--foreground` → `#f7f8f8`                                                                                |
| Secondary / silver              | `--ds-text-secondary` → `#d0d6e0`                                                                         |
| Tertiary muted                  | `--muted-foreground` → `#8a8f98`                                                                          |
| Brand / CTA fill                | `--primary` → `#5e6ad2`                                                                                   |
| Focus / ring / links            | `--ring` → `#7170ff`                                                                                      |
| Primary hover                   | `--ds-accent-hover` → `#828fff`                                                                           |
| Inter + OpenType                | `html` in `src/mainview/index.css`: `font-feature-settings: "cv01", "ss03"`                               |
| Monospace (no Berkeley bundled) | `--font-mono`: `ui-monospace, SF Mono, Menlo, monospace`                                                  |
...
| Date       | Decision                                                                                                                                                                                            |
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-04-13 | Replaced prior DESIGN.md with full Linear reference; mapped tokens to `#08090a` / `#0f1011` / `#191a1b`, accents `#5e6ad2` / `#7170ff` / `#828fff`.                                                 |
| 2026-04-13 | Switched app font from Plus Jakarta Sans to **Inter Variable** with `cv01`, `ss03`.                                                                                                                 |
| 2026-04-12 | **Accent presets** (indigo, coral, teal, amber, rose, mono) in Settings are intentional user preference overrides; default UI still maps to brand indigo/violet tokens in `src/mainview/index.css`. |
