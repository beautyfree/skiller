...
- **Release notes** — Release Please owns `CHANGELOG.md`. Every `dev`, `build`, and package command runs `bun run generate:release-notes`, which produces `src/mainview/release-notes.generated.json` for the renderer. Never edit that JSON by hand; add a conventional commit so the next release PR updates the changelog. The app shows the matching version once after an upgrade and keeps the history available in Settings.
- **Releases** — merging a Release Please PR creates a draft release, then directly calls the reusable `Release` workflow with that exact tag and commit SHA. It verifies, builds, uploads, and publishes the same draft. A normal `Release Please` run that creates no release does not start a packaging workflow. Use `Release` → **Run workflow** only to recover an existing draft by tag.
