...
Current foundation evidence (2026-08-03): the public repository is `https://github.com/beautyfree/dotagent`. The current core commit is `328bad8`; CI run `30814899193` completed the Ubuntu, macOS, and Windows jobs successfully. The same commit passes 104 tests, generated-schema drift checks, a committed API-declaration snapshot for 29 typed exports, and package inspection for 156 files/32 export paths locally. It includes the shared, runtime-validated import-decision contract (`owned`, `dependency`, `vendored`, `local-only`, `excluded`), deterministic transactional reconciliation, and atomic multi-file library updates used by Skiller. CI uses a frozen install with lifecycle scripts disabled and also runs inspect/audit smoke checks. `release:check` inspects the dry-run npm tarball and all exported package targets; the release-artifact builder produces the exact tarball, SHA-256 checksum, CycloneDX SBOM, and release manifest. A manual OIDC publish workflow exists, but npm publication remains intentionally blocked by `private: true` and the placeholder version until the stable-release gates are complete.
...
- [ ] Replace secret/source/integrity logic in vertical slices, one subsystem at a time. (Secret scanning, safe export policy, three-way classification, immutable dependency resolution, canonical Git workspace operations, bundled restore reconciliation, and atomic publish writes are shared; remaining Skiller-specific legacy/canonical manifest assembly and granular merge policy are isolated behind compatibility adapters.)
...
Current integration evidence (2026-08-03): dotagent owns the legacy Skiller schemas, migrations, portable-path checks, duplicate detection, credential-free remote validation, bundled-library reconciliation, and the multi-target publish transaction. Skiller retains compatibility facades so existing imports and repositories do not change. Restore maps the local ledger into the shared three-way plan and retains an isolated legacy engine behind an environment kill switch. Publish maps reviewed bundles and generated portable files into a shared library-update plan that validates an absent/existing root, stages every operation before the first replacement, rolls back the whole set, and exposes durable recovery through the existing tRPC recovery actions. Focused parity, deterministic-plan, ledger-conflict, stale-preview, atomic publish, and old/new journal recovery tests pass; full release/platform gates remain open.

Latest runtime evidence (2026-08-03): Skiller pins dotagent commit `328bad8`, including the authoritative 49-agent catalog, versioned schemas/fixtures, dependency audit deltas, shared portable-plus-local agent routing, reviewed clone/init/resolve/reconcile/library-update plans, shared owned-skill export policy, shared import decisions, third-party conformance fixtures, and the public API snapshot gate. Newly created Sync Center repositories use canonical `skills.json`, `skills.lock`, `dotagent.yaml`, and `skills/` content and delegate canonical Git operations, atomic publish writes, and bundled restore reconciliation to dotagent; existing `skiller-sync.yaml` repositories remain readable and writable through versioned adapters. The Skiller adapter tests prove serialized import-plan equality, restore-preview parity against the legacy engine, and deterministic publish workspace-plan mapping. The full 107-test suite, typecheck, and production Electron/Vite build pass after the current pin. A fresh live renderer review confirmed the three-step Sync Center, per-skill outcome controls, vendored-license disclosure, and license-gated continuation; native backend preview remains covered by integration tests because the standalone browser renderer has no Electron RPC bridge. Packaged-app verification remains a separate release gate.
