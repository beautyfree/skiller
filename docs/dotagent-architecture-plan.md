...
Current foundation evidence (2026-08-03): the public repository is `https://github.com/beautyfree/dotagent`. The current core commit is `fa24b2e`; its local release gate passes 108 tests with 382 assertions, generated-schema drift checks, a committed API-declaration snapshot for 30 typed exports, and package inspection for 160 files/33 export paths. CI run `30821036175` validates this exact commit successfully on Ubuntu, macOS, and Windows. The core includes the shared, runtime-validated import-decision contract (`owned`, `dependency`, `vendored`, `local-only`, `excluded`), deterministic transactional reconciliation, atomic multi-file library updates, identified Skiller compatibility publish plans, generic reviewed Git fast-forward plans, and stale-checked no-write recovery previews. CI uses a frozen install with lifecycle scripts disabled and also runs inspect/audit smoke checks. The release-artifact builder refuses dirty or mismatched source checkouts and produces byte-reproducible tarball, CycloneDX SBOM, checksums, changelog, migration/RFC guides, and a manifest binding npm integrity to the exact Git commit. Independent double-build comparison and tamper rejection pass locally. Non-publishing release workflow run `30821075183` independently rebuilt, verified, and uploaded artifact `8858854620` with archive digest `sha256:64d618f10f450164fefd65f1e559e3cda540e5895bd53d432150ba65f8d30f0d`; the publish job was intentionally skipped. A manual OIDC publish boundary exists, but npm publication remains blocked by `private: true` and the placeholder version until an explicitly reviewed release commit.
...
- [ ] Publish package provenance, checksums, SBOM, changelog, and migration guide. The complete byte-reproducible artifact set is generated, independently verified, tamper-tested, and retained by successful non-publishing workflow run `30821075183`; npm registry publication remains intentionally disabled until version/visibility and the release itself are explicitly approved.
- [x] Add public data-only extension points for new agent descriptors without arbitrary code execution.
- [x] Consider MCP/roles/hooks only after skills reach the stable acceptance bar. They remain deliberately deferred: v1 schemas and execution surfaces cover skills only and reserve no speculative hook or role fields.
