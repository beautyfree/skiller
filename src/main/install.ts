...
export function installSkillFromPath(
  ...
  for (const agent of targetAgents) {
    ...
    if (!readsShared) {
      ...
      if (!gp)
        throw new Error(`agent \`${slug}\` has no global paths configured`);
      const agentRoot = gp;
...
function deriveGitTargetSkillName(
  repoUrl: string,
  ...
  try {
    ...
    if (expectedContentHash) {
      ...
      if (sourcePlan.sha256 !== expectedContentHash) {
        throw new Error(
          `Pinned source content does not match the reviewed manifest: ${skillName}`,
        );
      }
    }
    return {
      ...
      repository: repoUrl,
      skillRelativePath,
      resolvedSha: checkout.resolvedCommit,
    };
  } catch (error) {
...
export function discardPreparedGitSkill(
  prepared: Pick<PreparedGitSkillInstall, "tempDir">,
): void {
  try {
...
export async function installSkillFromGit(
  ...
  expectedContentHash?: string,
  /** Exact device-owned trust and optional cooling-off decision reviewed before clone. */
  sourcePolicy: SourceSecurityPolicyInput = {},
): Promise<string> {
  const prepared = await prepareGitSkillInstall(
    repoUrl,
    skillRelativePath,
    ref,
    targetSkillName,
  ...
  try {
    return installPreparedGitSkill(
      prepared,
      targetAgentSlugs,
      agents,
      sourceLabel,
    ...
    try {
      ...
      /* discard is already failure-safe */
    }
  }
}
