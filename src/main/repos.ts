...
import {
  checkoutReviewedGitSource,
  fastForwardReviewedGitSource,
} from "./git-transport";
...
export async function addSkillRepo(
  ...
  if (existsSync(localPath)) {
    const settings = readSettings();
    const inConfig = settings.repos?.some(
      (r) => r.repo_url === normalizedRepoUrl,
    );
    if (inConfig) {
      ...
      repo.last_synced =
        settings.repos?.find((r) => r.repo_url === normalizedRepoUrl)
          ?.last_synced ?? null;
  ...
  try {
    await checkoutReviewedGitSource(
      normalizedRepoUrl,
      localPath,
      "HEAD",
      sourcePolicy,
...
export async function syncSkillRepo(
  ...
  const settings = readSettings();
  const remoteEntry = settings.repos?.find(
    (entry) => entry.repo_url && repoIdFromUrl(entry.repo_url) === repoIdParam,
  );
  const trustedRemote = remoteEntry?.repo_url;
  if (!trustedRemote)
    throw new Error("Repository has no reviewed remote source");
  emit({ stage: "fetching", detail: null });
  emit({ stage: "merging", detail: null });
  await fastForwardReviewedGitSource(localPath, sourcePolicy);
