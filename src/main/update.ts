...
export class RepoSession {
  ...
  static async open(
    ...
    if (existsSync(join(persistent, ".git"))) {
      await fastForwardReviewedGitSource(persistent, sourcePolicy);
    ...
    await checkoutReviewedGitSource(repoUrl, temp, "HEAD", sourcePolicy);
...
export async function updateSingleSkill(
  ...
  const session = await RepoSession.open(repoUrl, sourcePolicy);
  try {
...
export async function updateAll(
  ...
  for (const [, skills] of groups) {
    ...
    try {
      session = await RepoSession.open(repoUrl, sourcePolicy);
    } catch (e) {
